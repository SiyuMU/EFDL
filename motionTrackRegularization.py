...
def extract_deform(b_coef_file_format,time_index_x,time_index_y,length_scale_in_mm,fileScale,weight=None,sampleratio=5.,savepath=None,separate=False):
    if separate:
        invar_dict=[]
        outvar_dict=[]
    else:
...
def run(cfg: ModulusConfig) -> None:
    ...
    if time_index.shape[-1]>2:
        ...
        weight=np.ones_like(time_index[:,2])
    ...
    if volume_anchor_file is not None:
        if os.path.isfile(os.path.join(casePath,training_folder,volume_anchor_file)):
            with open(os.path.join(casePath,training_folder,volume_anchor_file),'r') as f:
                volume_anchor_file=f.readlines()
                print("anchor",volume_anchor_file)
            while volume_anchor_file[0][-1:] in ['\r','\n']:
                volume_anchor_file[0]=volume_anchor_file[0][:-1]
            volume_anchor_stltimefrom=float(volume_anchor_file[1])
            volume_anchor_stlfilefrom=os.path.join(casePath,volume_anchor_file[0].format(volume_anchor_stltimefrom))
            volume_anchor_stltimeto=float(volume_anchor_file[2])
            volume_anchor_stlfileto=os.path.join(casePath,volume_anchor_file[0].format(volume_anchor_stltimeto))
            volume_anchor_volumeratio=float(volume_anchor_file[3])
            if len(volume_anchor_file)>4:
                volume_anchor_lambda=float(volume_anchor_file[4])
            else:
                volume_anchor_lambda=1.
        else:
            raise Exception("Cannot find stl to load interior points.")
        interior_mesh_from = Tessellation.from_stl(
            volume_anchor_stlfilefrom, airtight=True,
        )
        volume_constrain_from = PointwiseInteriorConstraint(
            nodes=nodes,
            geometry=interior_mesh_from,
            outvar={'sum_det':volume_anchor_volumeratio},
            lambda_weighting={'sum_det':volume_anchor_lambda},
            parameterization=Parameterization({Symbol("t"): volume_anchor_stltimefrom,Symbol("Delta_t"): volume_anchor_stltimeto-volume_anchor_stltimefrom,}),
            batch_size=cfg.batch_size.anchor,
            batch_per_epoch= 1000,
            fixed_dataset = True,
            quasirandom =True,
        )
        domain.add_constraint(volume_constrain_from, "volume_from")
        interior_mesh_to = Tessellation.from_stl(
            volume_anchor_stlfileto, airtight=True,
        )
        volume_constrain_to = PointwiseInteriorConstraint(
            nodes=nodes,
            geometry=interior_mesh_to,
            outvar={'sum_det':1./volume_anchor_volumeratio},
            lambda_weighting={'sum_det':volume_anchor_lambda},
            parameterization=Parameterization({Symbol("t"): volume_anchor_stltimeto,Symbol("Delta_t"): volume_anchor_stltimefrom-volume_anchor_stltimeto,}),
            batch_size=cfg.batch_size.anchor,
            batch_per_epoch= 1000,
            fixed_dataset = True,
            quasirandom =True,
        )
        domain.add_constraint(volume_constrain_to, "volume_to")
        if smooth_time_file is not None:
            ...
            for tref in smooth_time_file:
                volume_constrain_from_ref0.append(PointwiseInteriorConstraint(
                    ...
                    lambda_weighting={'deviation_det':volume_anchor_lambda},
                    parameterization=Parameterization({Symbol("t"): volume_anchor_stltimefrom,Symbol("Delta_t"): tref-volume_anchor_stltimefrom,}),
                    batch_size=cfg.batch_size.anchor,
                    batch_per_epoch= 1000,
                    fixed_dataset = True,
                    quasirandom =True,
                ))
                domain.add_constraint(volume_constrain_from_ref0[-1], "volume_from_ref"+str(tref))
                volume_constrain_to_ref0.append(PointwiseInteriorConstraint(
                    nodes=nodes,
                    geometry=interior_mesh_to,
                    outvar={'deviation_det':0.},
                    lambda_weighting={'deviation_det':volume_anchor_lambda},
                    parameterization=Parameterization({Symbol("t"): volume_anchor_stltimeto,Symbol("Delta_t"): tref-volume_anchor_stltimeto,}),
                    batch_size=cfg.batch_size.anchor,
                    batch_per_epoch= 1000,
                    fixed_dataset = True,
                    quasirandom =True,
                ))
                domain.add_constraint(volume_constrain_to_ref0[-1], "volume_to_ref"+str(tref))
    ...
    if stlt0 is not None:
        if stlt0[-4:]=='.stl':
            ...
            stlt0=VTKPolyData.init_from_obj(stlt0[:-4],casePath)
        else:
